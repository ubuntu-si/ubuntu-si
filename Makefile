deb64:
	mkdir -p dist64
	cd si-ubuntu-defaults;	dpkg-buildpackage -d -us -uc -F -aamd64; cd .. 
	mv si-ubuntu-defaults_* dist64
deb32:
	mkdir -p dist32
	cd si-ubuntu-defaults;	dpkg-buildpackage -d -us -uc -F -ai386;	cd .. 
	mv si-ubuntu-defaults_* dist32
iso64:
	cd dist64/; ubuntu-defaults-image --release precise --arch amd64 --package si-ubuntu-defaults_*_all.deb --components main,restricted,universe
iso32:
